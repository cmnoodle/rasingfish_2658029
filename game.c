...
bool engine_init()
{
    ...
    // 물고기 텍스처 로드
    fishTextures[0] = loadTexture("fish.bmp");
    fishTextures[1] = loadTexture("fish_fast.bmp");
    fishTextures[2] = loadTexture("fish_big.bmp");
    fishTextures[3] = loadTexture("fish_slow.bmp");
    fishTextures[4] = loadTexture("fish_small.bmp");
    fishTextures[5] = loadTexture("fish_special.bmp");

    // 하나라도 로드에 실패했는지 체크
    for (int i = 0; i < NUM; i++) {
        if (!fishTextures[i]) {
            printf("%d번째 물고기 이미지 로드 실패!\n", i);
            SDL_Quit();
            return false;
        }
    }
...
void initGame() {
    for (int i = 0; i < NUM; i++)
    {
        ...
        case 0:
            ...
            fishTanks[i].growthRate = 1.0f;       // 표준 성장 (1배)
            fishTanks[i].consumptionRate = 1.0f;  // 표준 소비 (1배)
            break;
        case 1:
            fishTanks[i].type = FAST_FISH;
            fishTanks[i].growthRate = 1.8f;       // 빨리 자람 (1.8배)
            fishTanks[i].consumptionRate = 1.1f;  // 물 소비는 비슷 (1.1배)
            break;
        case 2:
            fishTanks[i].type = BIG_FISH;
            fishTanks[i].growthRate = 1.2f;       // 성장 보통 (1.2배)
            fishTanks[i].consumptionRate = 2.5f;  // 물을 엄청나게 마심 (2.5배)
            break;
        case 3:
            fishTanks[i].type = SLOW_FISH;
            fishTanks[i].growthRate = 1.0f;       // 표준 성장 (1배)
            fishTanks[i].consumptionRate = 0.3f;  // 물이 거의 안 줄어듦 (0.3배)
            break;
        case 4:
            fishTanks[i].type = SMALL_FISH;
            fishTanks[i].growthRate = 0.6f;       // 느리게 자람 (0.6배)
            fishTanks[i].consumptionRate = 0.3f;  // 물이 거의 안 줄어듦 (0.3배)
            break;
        case 5:
            fishTanks[i].type = SPECIAL_FISH;
            fishTanks[i].growthRate = 3.0f;       // 폭풍 성장 (3.0배)
            fishTanks[i].consumptionRate = 2.0f;  // 물 소비 많음 (2.0배)
            break;
        }
    }
...
void cleanupGame() {
    ...
    SDL_DestroyTexture(fishTextures);            // 물고기 텍스처 메모리 해제
    if (audioDevice != 0) {
...
void renderFishTanks() {                        // 어항과 물고기 렌더링 함수
    for (int i = 0; i < NUM; i++) {
        ...
        if (fishTanks[i].isAlive) {
            ...
            // 현재 어항의 물고기 타입(0~5번 정수)을 인덱스로 사용하여 알맞은 이미지를 전달합니다.
            int typeIndex = fishTanks[i].type;
            SDL_RenderCopy(renderer, fishTextures[typeIndex], NULL, &fishRect);
        }
        //물이 30이하 일 시 위험 표시
        if (fishTanks[i].isAlive && fishTanks[i].water <= 30) {
            if ((SDL_GetTicks() % 500) < 250) {
                renderText("!WARING!", x + 15, 270);
            }
        }
    ...
    if (elapsed > 0) {
        ...
        for (int i = 0; i < NUM; i++) {
            if (fishTanks[i].isAlive == 1) {
                aliveCount++;
                // 물 증발 및 소비
                fishTanks[i].water -= (level * (fishTanks[i].fish / 20 + 1) * elapsed) * fishTanks[i].consumptionRate;
                ...
                if (fishTanks[i].water > 0) {
                    // 기본 성장 수치 계산
                    float baseGrowth = ((fishTanks[i].water / 100.0f + 1) * elapsed) * fishTanks[i].growthRate;

                    // 물이 충분할 때(70 초과) 1.5배 보너스 적용
                    if (fishTanks[i].water > 70) {
                        baseGrowth *= 1.5f;
                    }

                    fishTanks[i].fish += baseGrowth;
                }
            }
        }
